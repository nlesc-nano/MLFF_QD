...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Plot Pool Active Learning Diagnostics from CSV")
    parser.add_argument("csv_path", type=str, help="Path to the al_pool_diagnostics.csv file")
    parser.add_argument("--out-dir", type=str, default="al_plots", help="Directory where output plots/html will be saved")
    parser.add_argument("--window", type=int, default=50, help="Smoothing window size for running mean")
    parser.add_argument("--dpi", type=int, default=300, help="DPI for saved PNG plots")
    parser.add_argument("--state", type=str, default=None, help="Optionally limit plotting to a specific state/head")
    parser.add_argument("--keep-first", action="store_true", help="Do not drop the first frame when calculating rolling mean")
    ...
    print(f"Generating diagnostic plots from {args.csv_path}...")
    outputs = generate_al_diagnostic_plots(
    ...
    print(f"Finished generating {len(outputs)} plots in '{args.out_dir}' directory.")
