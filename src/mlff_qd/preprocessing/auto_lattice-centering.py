...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Center and add cubic lattice to XYZ dataset (auto padding, non-periodic, silent)")
    parser.add_argument("input_file", help="Input .xyz file")
    parser.add_argument("--engine", choices=["nequip", "schnet", "allegro", "mace"], required=True, help="Engine type")
    parser.add_argument("--output", help="Output filename (default: auto)", default=None)
    ...
    if args.output:
        ...
        output_file = f"{base}_{args.engine}_centered.xyz"

    png_file = f"{base}_{args.engine}.png"

    process_xyz(args.input_file, output_file, png_file)
