...
def preprocess_data(data, use_last_n=1000):  # Default value as fallback
    numbers = data["z"]
    atoms_list = []
    property_list = []
    ...
    # Take the last use_last_n structures
    total_structures = len(positions_array)
    start_idx = max(0, total_structures - use_last_n)
    positions_array = positions_array[start_idx:]
    energies_array = energies_array[start_idx:]
    forces_array = forces_array[start_idx:]
    
    print(f'Energies Sample: {energies_array[-5:]}')
    ...
    logging.info(f"Using last {len(positions_array)} structures out of {total_structures}")
